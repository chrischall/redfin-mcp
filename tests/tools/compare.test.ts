...
describe('redfin_compare_properties stops issuing fetches after the deadline (fleet-audit #956)', () => {
  it('neither dequeues queued targets nor retries timed-out in-flight rows once the deadline answered pending', async () => {
    ...
    const h = await createTestHarness((server) =>
      registerCompareTools(server, mockClient, { overallDeadlineMs: 100 })
    );
    mockFetchStingrayJson.mockImplementation(() => gate.hold());
    const r = await h.callTool('redfin_compare_properties', {
    ...
    expect(callsAtDeadline).toBe(BRIDGE_CONCURRENCY * 2);
