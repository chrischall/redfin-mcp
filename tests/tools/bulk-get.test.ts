...
describe('redfin_bulk_get stops issuing fetches after the deadline (fleet-audit #956)', () => {
  it('neither dequeues queued targets nor retries timed-out in-flight rows once the deadline answered pending', async () => {
    const gate = createFetchGate();
    const h = await createTestHarness((server) =>
      registerBulkGetTools(server, mockClient, { overallDeadlineMs: 100 })
    );
    mockFetchStingrayJson.mockImplementation(() => gate.hold());
    const r = await h.callTool('redfin_bulk_get', {
    ...
    // 6 in-flight rows x (ATF + BTF).
    expect(callsAtDeadline).toBe(12);
    // The hung in-flight fetches now fail with a retryable bridge timeout.
